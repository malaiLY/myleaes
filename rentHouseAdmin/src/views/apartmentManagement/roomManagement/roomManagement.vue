...
const ROOM_LIST_PAGE_CACHE_KEY = 'room-management-page-num'
const getCurrentTablePage = () => {
  return (
    proTable.value?.pageable?.pageNum ||
    proTable.value?.pageable?.value?.pageNum ||
    1
  )
}
...
const editHandle = (row: RoomInterface) => {
  ...
  sessionStorage.setItem(
    ROOM_LIST_PAGE_CACHE_KEY,
    String(getCurrentTablePage()),
  )
  router.push({
    path: '/apartmentManagement/roomManagement/addOrEditRoom',
    query: {
      id: row.id,
      pageNum: String(getCurrentTablePage()),
    },
  })
}
// 新增
...
onMounted(() => {
  ...
  const cachedPageNum = sessionStorage.getItem(ROOM_LIST_PAGE_CACHE_KEY)
  const currentPage = Number(route.query.pageNum || cachedPageNum || 1)
  if (currentPage > 1) {
    nextTick(() => {
      proTable.value!.pageable.pageNum = currentPage
      proTable.value?.getTableList()
    })
  }
})
